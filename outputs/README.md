...
| `tables/` | One csv per analysis: the ECx estimates, and the NEC or NSEC estimate for `bayesnec` fits. Where more than one model contributed, a second csv, `<stem>_weights.csv`, records how much each candidate carried. |
...
| `reports/` | The rendered html workflow document, moved here by whichever entry point ran the analysis. |

File names follow the stem `<sample_id>_<test_type>_<engine>`, so the products
of one analysis sort together.
